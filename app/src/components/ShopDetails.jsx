...
const products = [
  {
    ...
    flavors: ["Jaggery", "Dates"],
    price: 68,
    rating: 5.0,
    reviews: 23,
    category: "Mixed Berry Zobo",
    tag: "Our Shop",
    mainImage: "/loveth-pack.png",
    thumbnails: [
      "/Rectangle 8884.png",
      "/Rectangle 8885.png",
      "/Rectangle 8886.png",
...
export default function ShopDetails() {
  ...
  // const router = useRouter();
  const flavorFromUrl = searchParams.get("flavor") || "Jaggery";
  const [selectedFlavor] = useState(flavorFromUrl);
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="w-full md:w-1/2 flex flex-col-reverse md:flex-row gap-4">
            ...
            <div
              ...
              style={{ height: "459px", borderRadius: "5px" }}
            >
              <Image
          ...
          <div className="w-full md:w-1/2">
            <div className="flex items-center gap-5 flex-wrap">
              <h1 className="text-3xl md:text-[48px] font-bold text-gray-900 mb-3">
                {product.name}
              </h1>
              <div className="flex items-center text-[18px] gap-3 text-sm text-gray-500 mb-3">
                <button
                  onClick={() => {
                    const prevId =
                      product.id === 1 ? products.length : product.id - 1;
                    router.push(`/shopDetails?id=${prevId}`);
                  }}
                  className="hover:text-[#AB0000] flex items-center gap-2 transition"
                >
                  <FaArrowLeft /> Prev
                </button>
                ...
                <button
                  onClick={() => {
                    const nextId =
                      product.id === products.length ? 1 : product.id + 1;
                    router.push(`/shopDetails?id=${nextId}`);
                  }}
                  className="flex items-center gap-2 hover:text-[#AB0000] transition"
                >
                  next <FaArrowRight />
                </button>
              </div>
            ...
            <p className="text-gray-500 text-base md:text-[18px] leading-relaxed mb-5">
              {product.desc}
            </p>

            {/* Flavor - read only, from URL */}
            <div className="mb-4">
              <p className="text-gray-700 font-semibold text-sm mb-2">
                Selected Flavor
              </p>
              <div className="border border-gray-300 bg-white px-4 py-2 w-fit text-gray-700 text-sm">
                {selectedFlavor}
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
              ...
              <button
                onClick={() => router.push("/checkout")} // ← add onClick
                className="w-full bg-[#9F2625] text-white font-semibold py-3 text-sm hover:bg-[#7a0d07] transition flex items-center justify-center gap-2"
              >
                Proceed to Checkout
                <svg
                  width="16"
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {[
                ...
                "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.742l7.747-8.856L1.506 2.25h6.964l4.255 5.643zm-1.161 17.52h1.833L7.084 4.126H5.117z",
                ...
                <a
                  key={i}
                  href="#"
                  className="w-8 h-8 rounded-full bg-[#4F4F4F] flex items-center justify-center hover:bg-[#AB0000] transition"
                >
          ...
          <div className="max-w-3xl">
            ...
            <ul className="space-y-1">
              {product.tabs.keyBenefits.map((b, i) => (
                ...
                >
                  <span className="text-[#AB0000] mt-0.5">•</span> {b}
                </li>
              ))}
            </ul>
          </div>
